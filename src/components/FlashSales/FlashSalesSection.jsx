  ...
  return (
    <section className="pb-[80px] overflow-hidden">
      <div className="container mx-auto relative px-2 sm:px-0">
        <motion.div initial={{y:-400,opacity:0.2}} animate={{y:0,opacity:1}} transition={{duration:0.4,delay:0.2}} className="flex flex-col gap-5  lg:flex-row lg:items-end lg:gap-[87px] mb-10">
          <SubTitle sectionTitle={"Today's"} title={"Flash Sales"} />
          <CountdownTimer deadline={deadline} />
        </motion.div>

        {isLoading ? <Loading type={2}/> : <Carousel Type={"card"} data={data} cartIsAdded={cartIsAdded} />}
